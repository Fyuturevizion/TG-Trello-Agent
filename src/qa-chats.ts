...
/** QA allowlist (env + KV) plus dojo forum chats. */
export async function loadAllowedChatIds(env: Env): Promise<number[]> {
  const qaIds = await loadQaChatIds(env);
  const dojoIds = parseDojoChatIds(env);
  return [...new Set([...qaIds, ...dojoIds])];
}
