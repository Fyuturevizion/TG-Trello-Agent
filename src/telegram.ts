import { isDojoChat, listQaChatIds, parseDojoChatIds } from './qa-chats';
...
export async function isAllowedChat(env: Env, chatId: number, chatType?: string): Promise<boolean> {
  if (isDojoChat(env, chatId)) return true;
  const qaIds = await listQaChatIds(env);
  const dojoIds = parseDojoChatIds(env);
  const configured = qaIds.length > 0 || dojoIds.length > 0;
  if (configured) return qaIds.includes(chatId) || dojoIds.includes(chatId);
